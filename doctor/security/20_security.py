"""Security surface test — consolidated (the Operator's 08-12 directive).

ONE module per category: this composer runs every security submodule's
...
def _load_sub(name: str):
    ...
    spec = importlib.util.spec_from_file_location(
        f"doctor.security._sub_{name}", str(path))
